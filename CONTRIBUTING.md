...
Thank you for your interest in contributing to **SessionPulse**! We welcome bug reports, feature requests, documentation improvements, and code contributions.
...
| :--- | :--- | :--- |
| `feat/` | New functionality | `feat/recurring-overtime` |
| `fix/` | Bug fixes | `fix/afk-detection-race` |
| `docs/` | Documentation only | `docs/config-reference` |
| `refactor/` | Restructuring with no behaviour change | `refactor/extract-session-tracker` |
| `perf/` | Performance work | `perf/reduce-timer-overhead` |
| `test/` | Tests only | `test/milestone-matching-edges` |
| `chore/` | Build, CI, tooling, dependencies | `chore/bump-adventure-platform` |

Rules:

* Lowercase, hyphen-separated, no underscores or spaces.
* Describe the change, not the author or the tool that made it — `feat/velocity-companion`,
...
1. **Fork & Clone**, and add this repository as `upstream` so you can keep `main` current:
   ...
   cd SessionPulse
   git remote add upstream https://github.com/Ninja6-MC/SessionPulse.git
   ```
...
5. **Commit Messages**:
   ...
   * `feat(milestones): add per-milestone sound override`
   * `fix(session): resolve AFK detection race on rejoin`
   * `docs(readme): add Spigot compatibility notes`
6. **Sign Off Every Commit**:
   ```bash
   git commit -s -m "fix(session): resolve AFK detection race on rejoin"
   ```
...
2. Open a Pull Request targeting the **`main`** branch of `Ninja6-MC/SessionPulse`.
...
SessionPulse is [GPL-3.0](LICENSE), and every contribution is published under that
